...
import top.zopx.goku.framework.primary.snowflake.util.FileUtil;
...
public class RegisterNodeService implements IRegisterNodeService {
    ...
    public int register(Node node) {
        ...
        // TODO 通过RPC技术，基于IP:PORT获取时间然后进行比对
        // 这里为了方便，获取临时列表进行比对
    ...
    private void checkCurrentTimeByNodeListData(List<Long> nodeList) {
        ...
        List<Long> list = nodeList.stream().filter(time -> -1L != time).collect(Collectors.toList());
